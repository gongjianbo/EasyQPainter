...
private:
    // 当前选区
    // QRect 有四个成员变量，分别对应左上角和右下角点坐标
    // x1 - 左上角坐标 x
    // x2 - 等于 x1+width-1
    // y1 - 左上角坐标 y
    // y2 - 等于 y1+height-1
    // 即 QRect(50,50,200,200) 时，topLeft=(50,50)bottomRight=(249,249)
    // fillRect 会填充整个区域
    // drawRect 在画笔宽度奇数时，右下角会多 1px，绘制时整体宽度先减去 1px
    QRect selection;
    // 是否有选区
