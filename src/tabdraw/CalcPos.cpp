...
CalcPos::CalcPos(QWidget *parent)
    : QWidget(parent)
{
    initImg_1();
    initImg_2();
...
void CalcPos::initImg_1()
{
    ...
    for (int i = 0; i < str_list.count(); i++)
    {
        ...
        painter.rotate(step); // 每次叠加旋转 30 度
    }
}

void CalcPos::initImg_2()
{
...
void CalcPos::draw_4(QPainter *painter)
{
    // 画一个逆时针的表盘
    QFont font;
    font.setFamily("SimSun");
...
void CalcPos::draw_5(QPainter *painter, int rotate)
{
    // 画一个顺时针的表盘
    // 用 qpainter 的 rotate，文字也是旋转的，可以想文字是固定方向
    ...
    for (int i = 0; i <= 180; i += step)
    {
        ...
        // x'=x0+R*cos(a)
        // y'=y0+R*sin(a)
        // 本来算出来是逆时针，不过因为屏幕坐标系 y 轴反的，就成了顺时针
        // x 或 y 取反都可以让表盘的数字顺序反一下，负负得正，但是左右上下颠倒了
        // 本来 xy 不取反，初始是 3 点方向顺时针往 9 点方向，现在为 9 点顺时到 3 点
