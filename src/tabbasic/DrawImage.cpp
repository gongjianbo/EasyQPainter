...
void DrawImage::paintEvent(QPaintEvent *event)
{
    ...
    // drawImage 是以左上角为起点，drawText 是以左下角为起点
    // 缩放
    // 平滑缩放效果会更好一点：scaled(w, h, Qt::IgnoreAspectRatio, Qt::FastTransformation)
    ...
    // 转换矩阵
    QTransform mat_a;      // 3*3 矩阵，貌似用 QMatrix 也可以
    ...
    // 可以发现，用矩阵转换的话就没法像转换 painter 坐标那样获取到矩形的实际边了
    // 一般我用 painter 的 rotate
    ...
    // 剪切变形，菱形的效果，为 0 时在原地，为 1 就拉伸了一倍的长/宽
    ...
    // 饱和度
    // HSL 格式表示色彩 - hue（色相）, saturation（饱和度）, lightness（明度）
    img_a = imgCache.copy();
