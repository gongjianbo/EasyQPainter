...
void PenStyle::paintEvent(QPaintEvent *event)
{
    ...
    // 准备路径
    // item 是元素可视大小, rect 是元素整体整体占位大小
    const int rect_width = this->width() / 3; //（3*2 个圆）
    ...
    // （如果线宽是奇数，直线抗锯齿会模糊，这是 Qt 的 bug）
    pen.setWidth(4);  // 线宽
    // 不开抗锯齿曲线有锯齿
    painter.setRenderHint(QPainter::Antialiasing);
    // 保存位置，等会儿换行用 restore 恢复，然后下移画第二行
    painter.save();
    ...
    // Qt::CustomDashLine
    // 除了要设置 pen style 为 Qt::CustomDashLine 外，
    // 还需要调用 setDashPattern 来描述自定义虚线的样子
    ...
    // 恢复位置准备画 cap 和 join 示意
    painter.restore();
    const int line_width = 10;
    path = QPainterPath(); // 低版本没有 clear
