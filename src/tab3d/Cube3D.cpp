...
void Cube3D::paintEvent(QPaintEvent *event)
{
    ...
    // 计算顶点变换后坐标，包含 z 值 max 点就是正交表面可见的，
    // 再计算下远小近大的透视投影效果齐活了
    QList<QVector3D> vertex_list; // 和矩阵运算后的顶点
    QList<int> vertex_max_list;   // z 最大值列表（z 值可能重复），内容为 vertexArr 的下标
    float vertex_max_value;       // 顶点列表 z 最大值
    // 根据旋转矩阵计算每个顶点
    ...
    // 绘制 front 和 back 六个面，先计算路径再绘制
    QList<QPainterPath> element_path_list; // 每个面路径
    QList<float> element_z_values;         // 每个面中心点的z值
    QList<QPointF> element_z_points;       // 每个面中心点在平面对应 xy 值
    QList<int> element_front_list;         // elementArr 中表面的 index
    // 计算每个表面
